...
This Repository contains information from Linkedin
 ...
 APPLYING THE CODE AGAIN LIKE FROM PREVIOUS CLASS, STILL USING THE IDE/NOT VSCODE YET, STEPS TO SSHING INTO ANSIBLE SERVER, GENERATING THE Ansible.cfg file IN ANSIBLE CONTROL SERVER, FORKS, USING THE APT/YUM MODULE TO INSTALL PACKAGE MANAGERS,  ROOT PRIVILEDGES   "-b",  TROUBLESHOOTING ; Apache not part of yum repo, IDEM POTENCY, 
 REMOVING A PACKAGE, USING YUM, SERVICE MODULE, USER MODULE, ENCRYPTION , SUDO !!, CREATING A uSER & ENCRYPTING THE PASSWD USING OPENSSL, SETUP MODULE, 
USING THE SETUP MODULE & VARIABLES, USING VARIABLES FOR APT/YUM MODULE, DISABLING SETUP MODULE, DEBUG MODULE,  ISSUSES WITH OPENSSL, CUSTOM AMIs, 
ANSIBLE PLAYBOOKS (are written in YAML) task run frm top to bottom, PLAYBOOK SYNTAX, EACH PLAY DEFINES 2 THINGS, ANSIBLE TASK EXECUTION, YAML BASICS, LIST, DICTIONARY, 
REMOTE DEVELOPMENT/ THE SECOND PART; SSH CONFIGURATION, MANUAL REMOTE CONFIGURATION, CONNECT TO HOST **mee, ie the control server ec2instance IN THE VSCODE TO START WRITING PLAYBOOKS, CREATED A DIR (ANSIBLE) TO CLONE THE ansible series repo, CLONING THE ANSIBLE SERIES REPO, CREATING ANOTHER DIR (testing)to WRITE FILES/PLAYBOOKS, HOW TO WIRTE PLAYBOOK, RUN THE PLAYBOOK, ALTHOUGH THE PLAYBK HAD ONE TASK BUT WE SEE 2TASKS THAT RAN ie the setup default module, MODIFYING THE PLAYBK TO GIVE IT A NAME, 
DISABLING THE SETUP MODULE, ADDING A SERVICE MODULE TO THE PLAYBOOK, USING BECOME ON THE HOST LEVEL INORDER TO REDUCE LINES OF CODE, EXPLAINING MORE PLAYBOOKS, 
PLAYBOOK TO INSTALL HTTPD ON A WEB REPO/SAMPLE PLAYBOOKS, CAPTURING INFO INSIDE A VAR CALLED RESULTS & USE THE DEBUG MODULE TO DISPLAY THE RESULT, 
USING CONDITIONS TO INSTALL HTTPD OR APACHE, USING CONDITIONS GATHERING FACTS HAS TO BE TRUE OR RUN A SHELL SCRIPT/COMMAND, 
 HOW TO SUCCESSFUL RUN PLAYBOOKS WHEN GATHER FACT IS false BY RUNNING A SHELL SCRIPT/COMMAND, WHEN GATHERING FACTS is TRUE, ill playbook will be shorten, WHAT ARE HANDLERS,
 TEMPLATE MODULE/JINJA2, WE HAVE A PROJECT THAT WILL REPLACE THE USER, CHANGE THE PW & SETUP USERS, LOOPS .
